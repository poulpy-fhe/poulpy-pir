...
use poulpy_hal::{
    api::{
        ...
        VecZnxBigBytesOf, VecZnxBigNormalize, VecZnxCopyBackend, VecZnxDftApply, VecZnxDftBytesOf,
        VecZnxIdftApply, VmpApplyDftToDft,
    },
    layouts::{
    ...
    VecZnx<BE::OwnedBuf>: VecZnxToBackendMut<BE> + VecZnxToBackendRef<BE>,
{
    ...
    let mut full = module.glwe_alloc_from_infos(&dst_infos);
    ...
    assert_eq!(split.data().raw(), full.data().raw());
}

#[test]
fn sequential_keyswitch_collapse_split_matches_full() {
    run::<FFT64Ref>();
}
