    ...
    VecZnx<BE::OwnedBuf>: VecZnxToBackendMut<BE> + VecZnxToBackendRef<BE>,
{
    ...
    let mut precomputed = module.glwe_alloc_from_infos(&dst_infos);
    sequential_keyswitch_collapse_aggregate_mask_precomputed(
        &module,
        &mut precomputed,
        lwe_matrix.body(),
    ...
    assert_eq!(precomputed.data().raw(), split.data().raw());
}

#[test]
fn precomputed_sequential_collapse_matches_split_baseline() {
    run::<FFT64Ref>();
}
