use crate::common::{scalar_mut, scalar_ref, split_output_key, vec_mut};
use poulpy_core::{
    EncryptionLayout, GLWEEncryptSk, GLWEExpandLWEMatrix, GLWEKeyswitch, GLWESwitchingKeyEncryptSk,
    GLWEZero,
    layouts::{
        Base2K, Degree, GGLWEPreparedFactory, GLWELayout, GLWESecret, GLWESecretPreparedFactory,
        GLWESwitchingKey, GLWESwitchingKeyLayout, GLWEToBackendRef, LWEInfos, LWEMatrixLayout,
...
use poulpy_pir::circuit::{
    ...
    sequential_collapse_mask_precompute_alloc, sequential_keyswitch_collapse_aggregate_mask_split,
    ...
    VecZnx<BE::OwnedBuf>: VecZnxToBackendMut<BE> + VecZnxToBackendRef<BE>,
{
    ...
    let mut split = module.glwe_alloc_from_infos(&dst_infos);
    ...
    let mut expected_mask = module.vec_znx_alloc(1, matrix_infos.size());
    {
        let split_ref = split.to_backend_ref();
        let mut expected_mut = vec_mut::<BE>(&mut expected_mask);
        module.vec_znx_copy_backend(&mut expected_mut, 0, split_ref.data(), 1);
    }

    assert_eq!(precompute.final_mask().raw(), expected_mask.raw());
}

#[test]
fn precompute_mask_state_matches_split_baseline() {
    run::<FFT64Ref>();
}
