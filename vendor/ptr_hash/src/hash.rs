...
impl<Key: KeyT + ?Sized> KeyHasher<Key> for Xxh3_128 {
    ...
    fn hash(x: &Key, seed: u64) -> u128 {
        let mut hasher = FixedWidth(xxhash_rust::xxh3::Xxh3Default::default());
        x.hash(&mut hasher);
        hasher.0.digest128() ^ (seed as u128 | (seed as u128) << 64)
    }
}

/// Forwards to `H`, but writes `usize`/`isize` at a fixed 64-bit width.
///
/// FORK: `Hash for [T]` prefixes the length via `write_usize`, whose default
/// writes `size_of::<usize>()` native-endian bytes. A 64-bit server and a
/// 32-bit wasm client therefore hash the same key to different values, which
/// makes the whole MPHF unportable. Everything else is forwarded unchanged.
struct FixedWidth<H>(H);

impl<H: std::hash::Hasher> std::hash::Hasher for FixedWidth<H> {
    #[inline(always)]
    fn finish(&self) -> u64 {
        self.0.finish()
    }
